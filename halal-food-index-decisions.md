...
Keep in the repo, and upload to any new conversation as context.

> **This supersedes the brand direction in the master production prompt.**
> That document (Part I, §4 "Existing technology and design direction") describes
> a navy hexagon mark, League Spartan and DM Sans, and emerald `#10B981`. All of
> that is retired. The strategy, engineering spec and phased plan in the master
> prompt still stand — only the visual direction has moved on. Where the two
> disagree on anything visual, this file wins.
...
**Wordmark** — "The Halal Food Index", set as one line, one weight, one size,
everywhere. No demoted "The", no stacked kicker. One lockup, used identically
in the header, footer and menu.

> Superseded 31 July 2026: the original version demoted "The" to a small
> kicker, stacked above the wordmark and pulled out of flow with
> `position:absolute` in the header so the wordmark would land on the nav
> line. In practice that meant only the wordmark's single line counted
> toward vertical centring against the seal, so the two-line lockup always
> read as sitting low in the circle — a layout bug, not a look. Fixed by
> removing the special-casing rather than patching it: the seal now centres
> against one line of text, the same as any ordinary logo, and the header,
> footer and menu need only one CSS rule between them instead of two. This
> also brings the site lockup in line with the OG card templates
> (`halal-food-index-social-cards.html`), which already set the brand as a
> single uniform line and turned out to have called this correctly first.
...
`halal-food-index-homepage-v10.html` is the reference build. One responsive file.
...
4. **Chain page** — `/is-nandos-halal`. Highest volume, biggest share value. Reference
   build exists (`halal-food-index-chain-nandos-v3.html`) — Nando's, all 16 spec
   elements from the master prompt §24, plus JSON-LD (ClaimReview, Dataset, Q&amp;A) and
   an AI-crawler-aware `robots.txt`. Still needs: real branch data, the other nine
   chains from §23's priority order, and translating into actual Next.js components.
5. **Venue page**.
